...
replace universitario = . if p10a == . & anio != "2001_" 
replace universitario = . if p10a_2001 == . & anio == "2001_" 

drop if universitario == .
...
* Definir etiquetas que serán usadas en los gráficos

label define rama_graph 1 "Agricultura" 3 "Manufactura" 6 "Construcción" 7 "Comercio" 9 "Alojamiento y comidas"
...
graph bar (mean) uni_crecimiento nouni_crecimiento, ///
	...
	name(crec_rama_2001_2010, replace)
...
graph bar (mean) uni_crecimiento_2 nouni_crecimiento_2, ///
    over(rama1, label(angle(45) labsize(small))) ///
    legend(order(1 "Universitarios" 2 "No universitarios")) ///
	name(crec_rama_2011_2024, replace)
